...
void gauss_seidel(const double hx, const double hy, const int nx, const int ny, const double *f, double* u){
    //u_k, u_{k+1}
    double *in,*out;
    ...
    for (int k = 0; k < 50000; k++) {
        // sweep
        /* no (trivial) parallelization possible because of dependencies (left/up neighbors) !! */
...
void redblack_gauss_seidel(const double hx, const double hy, const int nx, const int ny, const double *f, double* u){
    //u_k, u_{k+1}
    double *in; //,*out;
    in = (double*) calloc(nx * ny, sizeof(double));
//    out = (double*) calloc(nx * ny, sizeof(double));
...
void jacobi(const double hx, const double hy, const int nx, const int ny, const double *f, double* u)
{
    //u_k, u_{k+1}
    double *in,*out;
    in = (double*) calloc(nx * ny, sizeof(double));
...
int main(int argc,char** argv)
{
    ...
    printf("JAC\tElapsed:\t %2.10f\n",elapsed);

    err = computeErr(u,uex,nx,ny);
    printf("Error:\t %f\n", err);
    ...
    clock_gettime(CLOCK_MONOTONIC,&start);
    gauss_seidel(hx,hy,nx,ny,f,u);
    clock_gettime(CLOCK_MONOTONIC,&stop);

    elapsed=(stop.tv_sec-start.tv_sec)+(stop.tv_nsec-start.tv_nsec)/1e9;
    printf("GS\tElapsed:\t %2.10f\n",elapsed);
    ...
    clock_gettime(CLOCK_MONOTONIC,&start);
    redblack_gauss_seidel(hx,hy,nx,ny,f,u);
    clock_gettime(CLOCK_MONOTONIC,&stop);

    elapsed=(stop.tv_sec-start.tv_sec)+(stop.tv_nsec-start.tv_nsec)/1e9;
    printf("RB\tElapsed:\t %2.10f\n",elapsed);
